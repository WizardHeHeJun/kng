...
This produces `skill-registry.yaml` with a `skills:` section auto-populated from the skill files.
...
- **禁止在项目目录或当前工作目录创建中间文件。** 初始化过程中需要的临时脚本或缓存数据必须写入 `${KNG_HOME}/cache/` 目录（不存在则先创建）。流程结束后应清理不再需要的缓存文件。
